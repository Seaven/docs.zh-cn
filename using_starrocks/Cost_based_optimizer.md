...
* 抽样收集：
    ...
    会均匀的从每一个partition中抽取N行数据进行统计信息计算，抽样行数可以通过参数指定。优点在于收集任务消耗的资源少，速度快，缺点在于收集的统计信息不准确，对优化器的帮助有限。默认进行抽样收集，抽样的行数默认为20万行，收集周期为1天，数据未更新不会重新收集。抽样收集可以通过手动或者定期的方式进行主动触发。

* 全量收集

    使用整个表的所有数据计算统计信息。优点在于收集到的统计信息准确，优化器可以更好的评估执行计划，但是缺点也比较明显，收集任务消耗资源非常大，速度慢。默认不会进行全量收集，用户可以按需对部分表进行全量收集。全量收集可以使用手动或者定期的方式进行主动触发。
...
* Analyze调度策略：
  ...
  * 定期Analyze收集：每张表收集的间隔默认为一天，可以通过命令参数`update_interval_sec`控制频率。默认每2小时检查一次是否有需要收集的表。
